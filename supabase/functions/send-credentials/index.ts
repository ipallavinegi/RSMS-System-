...
Deno.serve(async (req) => {
  ...
  try {
    const { userEmail, userName, password, role } = await req.json();
    ...
    const mailOptions = {
      ...
      text: `Hello ${userName},\n\nYour account has been created for the role of ${role}.\n\nYour login password is: ${password}\n\nPlease log into the app using your email address and this password.`,
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px;">
          ...
          <p style="font-size: 16px; color: #555;">Your temporary login password is:</p>
          <div style="background-color: #f5f5f5; padding: 12px 24px; font-size: 24px; font-weight: bold; letter-spacing: 2px; color: #0046c0; border-radius: 6px; display: inline-block; margin: 12px 0;">
            ${password}
          </div>
          <p style="font-size: 14px; color: #777;">Please log into the app using your email address and this password.</p>
        </div>
      `,
    };
